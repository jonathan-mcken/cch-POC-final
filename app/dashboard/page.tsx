...
export default function DashboardPage() {
      ...
      try {
        ...
        if (statusData.success && statusData.summary) {
          ...
          if (summary.isComplete) {
            ...
            const filesResponse = await fetch(`/api/tax/batch-output-files?batchGuid=${encodeURIComponent(executionId)}`, {
              method: 'GET',
            });
            ...
            setExportProgress(`⏳ Batch processing... ${progressInfo?.completionPercentage || 0}% complete (${progressInfo?.completedItems || 0}/${progressInfo?.totalItems || 0} items)`);
  ...
  const downloadAllFiles = async (executionId: string, files: any[]) => {
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const downloadUrl = `/api/tax/download-file?` + new URLSearchParams({
  ...
  const downloadFileForItem = async (item: any) => {
    ...
    try {
      ...
      setExportProgress(`❌ Error downloading file: ${downloadError instanceof Error ? downloadError.message : String(downloadError)}`);
    } finally {
  ...
  const processZipFile = async (arrayBuffer: ArrayBuffer, fileName: string) => {
    try {
      ...
      const xmlFiles = Object.keys(zipContents.files).filter(name => 
  ...
  const fetchFinancialData = async (userId: string, beginDate = '2024-01-01', endDate = '2024-12-31') => {
    try {
      ...
      setV2CreationProgress(`❌ Error fetching financial data: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  };
  ...
  const reconstructXmlWithNewData = async (templateXml: string, accountData: any, profitLossData: any): Promise<string> => {
    try {
      ...
      for (let i = 0; i < Math.min(3, templateLines.length); i++) {
        ...
        if (i === 1 && templateLines[i].length >= 17) {
          console.log(`Template Line 2, position 17: "${templateLines[i].charAt(16)}" (char code: ${templateLines[i].charCodeAt(16)})`);
        }
      }
  ...
  const mapAccountDataToXml = (xml: string, accountData: any): string => {
    try {
      ...
      if (accountData.companyName) {
        ...
        xml = xml.replace(/Value="[^"]*" LocationType="FieldID" Location="SFDSBAGN\.0"/, `Value="${sanitizeXmlValue(accountData.companyName)}" LocationType="FieldID" Location="SFDSBAGN.0"`);
      }
      ...
      if (accountData.ownerName) {
        ...
        xml = xml.replace(/Location="SFDSSHNM\.0" LocationType="FieldID" Value="[^"]*"/, `Location="SFDSSHNM.0" LocationType="FieldID" Value="${sanitizeXmlValue(firstName)}"`);
        xml = xml.replace(/Location="SFDSSHNM\.18" LocationType="FieldID" Value="[^"]*"/, `Location="SFDSSHNM.18" LocationType="FieldID" Value="${sanitizeXmlValue(lastName)}"`);
        
        // Update officer compensation name
        xml = xml.replace(/Location="BFDSCMPO\.1" LocationType="FieldID" Value="[^"]*"/, `Location="BFDSCMPO.1" LocationType="FieldID" Value="${sanitizeXmlValue(firstName)}"`);
        xml = xml.replace(/Location="BFDSCMPO\.3" LocationType="FieldID" Value="[^"]*"/, `Location="BFDSCMPO.3" LocationType="FieldID" Value="${sanitizeXmlValue(lastName)}"`);
      }
      
      // Email
      if (accountData.ownerEmail) {
        xml = xml.replace(/Location="SFDSBAGN\.32" LocationType="FieldID" Value="[^"]*"/, `Location="SFDSBAGN.32" LocationType="FieldID" Value="${sanitizeXmlValue(accountData.ownerEmail)}"`);
      }
      ...
      if (accountData.homeAddress) {
        ...
        if (address.street1) {
          xml = xml.replace(/Location="SFDSBAGN\.3" LocationType="FieldID" Value="[^"]*"/, `Location="SFDSBAGN.3" LocationType="FieldID" Value="${sanitizeXmlValue(address.street1)}"`);
          xml = xml.replace(/Location="BFDSCMPO\.4" LocationType="FieldID" Value="[^"]*"/, `Location="BFDSCMPO.4" LocationType="FieldID" Value="${sanitizeXmlValue(address.street1)}"`);
          xml = xml.replace(/Location="SFDSSHNM\.2" LocationType="FieldID" Value="[^"]*"/, `Location="SFDSSHNM.2" LocationType="FieldID" Value="${sanitizeXmlValue(address.street1)}"`);
        }
        if (address.city) {
          xml = xml.replace(/Location="SFDSBAGN\.4" LocationType="FieldID" Value="[^"]*"/, `Location="SFDSBAGN.4" LocationType="FieldID" Value="${sanitizeXmlValue(address.city)}"`);
          xml = xml.replace(/Location="BFDSCMPO\.5" LocationType="FieldID" Value="[^"]*"/, `Location="BFDSCMPO.5" LocationType="FieldID" Value="${sanitizeXmlValue(address.city)}"`);
          xml = xml.replace(/Location="SFDSSHNM\.3" LocationType="FieldID" Value="[^"]*"/, `Location="SFDSSHNM.3" LocationType="FieldID" Value="${sanitizeXmlValue(address.city)}"`);
        }
        if (address.state) {
          xml = xml.replace(/Location="SFDSBAGN\.5" LocationType="FieldID" Value="[^"]*"/, `Location="SFDSBAGN.5" LocationType="FieldID" Value="${sanitizeXmlValue(address.state)}"`);
          xml = xml.replace(/Location="BFDSCMPO\.26" LocationType="FieldID" Value="[^"]*"/, `Location="BFDSCMPO.26" LocationType="FieldID" Value="${sanitizeXmlValue(address.state)}"`);
          xml = xml.replace(/Location="SFDSSHNM\.19" LocationType="FieldID" Value="[^"]*"/, `Location="SFDSSHNM.19" LocationType="FieldID" Value="${sanitizeXmlValue(address.state)}"`);
        }
        if (address.zip) {
          xml = xml.replace(/Location="SFDSBAGN\.6" LocationType="FieldID" Value="[^"]*"/, `Location="SFDSBAGN.6" LocationType="FieldID" Value="${sanitizeXmlValue(address.zip)}"`);
          xml = xml.replace(/Location="BFDSCMPO\.7" LocationType="FieldID" Value="[^"]*"/, `Location="BFDSCMPO.7" LocationType="FieldID" Value="${sanitizeXmlValue(address.zip)}"`);
          xml = xml.replace(/Location="SFDSSHNM\.20" LocationType="FieldID" Value="[^"]*"/, `Location="SFDSSHNM.20" LocationType="FieldID" Value="${sanitizeXmlValue(address.zip)}"`);
        }
      }
  ...
  const mapFinancialDataToXml = (xml: string, profitLossData: any): string => {
    try {
      ...
      xml = xml.replace(/Location="BFDSBINC\.3" LocationType="FieldID" Value="[^"]*"/, `Location="BFDSBINC.3" LocationType="FieldID" Value="${sanitizeXmlValue(totalIncome)}"`);
      
      // Officer wages - Compensation of Officers
      const officerWages = data.expenses?.children?.find((exp: any) => exp.name === 'Officer Wages & Salaries')?.total || 0;
      xml = xml.replace(/Location="BFDSCMPO\.13" LocationType="FieldID" Value="[^"]*"/, `Location="BFDSCMPO.13" LocationType="FieldID" Value="${sanitizeXmlValue(officerWages)}"`);
      ...
      xml = xml.replace(/RowValue Value="12509" FieldIndex="6"/, `RowValue Value="${sanitizeXmlValue(payrollTax)}" FieldIndex="6"`);
      
      // State tax - in the taxes section  
      const stateTax = expenses.find((exp: any) => exp.name === 'State Tax')?.total || 0;
      xml = xml.replace(/RowValue Value="7517" FieldIndex="6"/, `RowValue Value="${sanitizeXmlValue(stateTax)}" FieldIndex="6"`);
      ...
      xml = xml.replace(/RowValue Value="1990" FieldIndex="5"/, `RowValue Value="${sanitizeXmlValue(mileageReimbursement)}" FieldIndex="5"`);
      xml = xml.replace(/RowValue Value="7986" FieldIndex="5"/, `RowValue Value="${sanitizeXmlValue(officeExpenses)}" FieldIndex="5"`);
      
      // Update Other Deductions total
      const otherDeductionsTotal = mileageReimbursement + officeExpenses;
      xml = xml.replace(/Location="BFDSBODD\.2" LocationType="FieldID" Value="[^"]*"/, `Location="BFDSBODD.2" LocationType="FieldID" Value="${sanitizeXmlValue(otherDeductionsTotal)}"`);
      
      // Update cash on balance sheet (estimate based on net income)
      const cashAmount = Math.round(data.netIncome * 0.076);
      xml = xml.replace(/Location="BFDSBSCA\.1" LocationType="FieldID" Value="[^"]*"/, `Location="BFDSBSCA.1" LocationType="FieldID" Value="${sanitizeXmlValue(cashAmount)}"`);
      
      // Update payroll wages payable (estimate based on payroll tax)
      const payrollWagesPayable = Math.round(payrollTax * 0.03);
      xml = xml.replace(/RowValue Value="378" FieldIndex="6"/, `RowValue Value="${sanitizeXmlValue(payrollWagesPayable)}" FieldIndex="6"`);
      
      // Update distribution amount (estimate based on net income)
      const distributionAmount = Math.round(data.netIncome * 0.928);
      xml = xml.replace(/Location="SFDSDIST\.0" LocationType="FieldID" Value="[^"]*"/, `Location="SFDSDIST.0" LocationType="FieldID" Value="${sanitizeXmlValue(distributionAmount)}"`);
  ...
  const sendTaxReturnToCCHAxcess = async (xmlContent: string): Promise<void> => {
    try {
      ...
      let processedContent = xmlContent.replace(/^<\?xml[^>]*\?>\s*/, '');
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4">
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">CCH Axcess Dashboard</h1>
            <p className="text-gray-600 dark:text-gray-400">OAuth authentication successful</p>
          </div>
          <div className="flex space-x-2">
            ...
            >
              ...
              <span>{refreshing ? 'Refreshing...' : 'Refresh Tokens'}</span>
            </Button>
            <Button
              onClick={logout}
              variant="destructive"
            ...
            <CardContent className="space-y-4">
              {user ? (
                <div className="space-y-3">
                  ...
                  {user.exp && (
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium">ID Token Expires:</span>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center space-x-2">
              ...
              <span>API Test with Auto-Refresh</span>
            </CardTitle>
            <CardDescription>
              Test authenticated API requests with automatic token refresh
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <Button
            ...
            >
              ...
              <span>{testingApi ? 'Testing...' : 'Test API with Auto-Refresh'}</span>
            </Button>
        ...
        {batchGuid && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Check the current status of batch processing
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex items-center space-x-2">
              ...
              {batchStatusResult && (
                <div className="space-y-2">
                  {batchStatusResult.success && batchStatusResult.summary && (
                    <div className="space-y-2">
                      <div className={`text-sm ${batchStatusResult.summary.isComplete ? 'text-green-600 dark:text-green-400' : 
                                                 batchStatusResult.summary.hasFailed ? 'text-red-600 dark:text-red-400' : 
                                                 'text-yellow-600 dark:text-yellow-400'}`}>
                        <p className="font-medium">
                          {batchStatusResult.summary.isComplete ? '✅ Batch completed!' : 
                           batchStatusResult.summary.hasFailed ? '❌ Batch failed!' : 
                           '⏳ Batch processing...'}
                        </p>
                        ...
                        {batchStatusResult.summary.progress && (
                          <div className="mt-2 space-y-1">
                            ...
                            <p>Items: {batchStatusResult.summary.progress.completedItems}/{batchStatusResult.summary.progress.totalItems}</p>
                        ...
                        {batchStatusResult.summary.items && batchStatusResult.summary.items.length > 0 && (
                          <div className="mt-2 text-xs space-y-1">
                            ...
                            {batchStatusResult.summary.items.map((item: any, index: number) => {
                              ...
                              return (
                                <div key={item.itemGuid || index} className="ml-2 p-1 border-l-2 border-gray-300 dark:border-gray-600">
                                  <div className="flex justify-between items-start">
                                    <div className="flex-1">
                                      <p>Status: {item.itemStatusDescription} ({item.itemStatusCode})</p>
                                      <p>Response: {item.responseDescription} ({item.responseCode})</p>
                                      {item.returnInfo?.ReturnId && (
                                        <p>Return ID: {item.returnInfo.ReturnId}</p>
                                      )}
                                    ...
                                    {isItemComplete && (
                                      ...
                                      >
                                        <Download className={`h-3 w-3 mr-1 ${downloadingFiles.has(`${item.itemGuid}-download`) ? 'animate-spin' : ''}`} />
                                        {downloadingFiles.has(`${item.itemGuid}-download`) ? 'Processing...' : 'Process XML'}
                                      </Button>
                                    )}
                                  </div>
                                  {isItemComplete && (
                  ...
                  <details className="mt-3">
                    ...
                    <div className="mt-2 bg-gray-100 dark:bg-gray-800 p-3 rounded text-sm">
                      <pre className="whitespace-pre-wrap">
                        {JSON.stringify(batchStatusResult, null, 2)}
                      </pre>
                    </div>
                  </details>
                </div>
        ...
        {showXmlContent && xmlContent && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Formatted XML content from: {xmlFileName}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="text-xs text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 p-3 rounded">
                ...
                <div>• <span className="text-purple-600 dark:text-purple-400">Purple</span>: XML declaration</div>
                <div>• <span className="text-blue-600 dark:text-blue-400">Blue</span>: Opening tags</div>
                <div>• <span className="text-red-600 dark:text-red-400">Red</span>: Closing tags</div>
                <div>• <span className="text-green-600 dark:text-green-400">Green</span>: Attributes</div>
                <div>• <span className="text-gray-800 dark:text-gray-200">Gray</span>: Content values</div>
              </div>
            </CardContent>
          </Card>
        )}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center space-x-2">
              <FileDown className="h-5 w-5" />
              <span>V2 Tax Return Creation</span>
            </CardTitle>
            <CardDescription>
              Create a new tax return using data from Formations and Hurdlr
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center space-x-4">
              ...
              >
                ...
                {isCreatingV2 ? 'Creating V2...' : 'Create V2 Tax Return'}
              </Button>
                    ...
                    {accountData.data?.homeAddress && (
                      <div className="col-span-2">
                        ...
                        <p className="text-gray-600 dark:text-gray-400">
                          {accountData.data.homeAddress.street1}<br/>
                          {accountData.data.homeAddress.city}, {accountData.data.homeAddress.state} {accountData.data.homeAddress.zip}
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <CardContent className="space-y-2">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600 dark:text-gray-400">
                        {financialData.metadata?.retrievedAt ? new Date(financialData.metadata.retrievedAt).toLocaleString() : 'N/A'}
                      </p>
                    </div>
                  </div>
        ...
        {importBatchGuid && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Check if your tax return was successfully imported into CCH Axcess
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex items-center space-x-2">
                ...
                <div className="text-sm text-gray-600 dark:text-gray-400">
                  Import Batch GUID: {importBatchGuid}
                </div>
              </div>
              ...
              {importBatchStatusResult && (
                <div className="space-y-2">
                  {importBatchStatusResult.success && importBatchStatusResult.summary && (
                    <div className="space-y-2">
                      <div className={`text-sm ${importBatchStatusResult.summary.isComplete ? 'text-green-600 dark:text-green-400' : 
                                                 importBatchStatusResult.summary.hasFailed ? 'text-red-600 dark:text-red-400' : 
                                                 'text-yellow-600 dark:text-yellow-400'}`}>
                        <p className="font-medium">
                          {importBatchStatusResult.summary.isComplete ? '✅ Import completed!' : 
                           importBatchStatusResult.summary.hasFailed ? '❌ Import failed!' : 
                           '⏳ Import processing...'}
                        </p>
                        ...
                        {importBatchStatusResult.summary.progress && (
                          <div className="mt-2 space-y-1">
                            ...
                            <p>Items: {importBatchStatusResult.summary.progress.completedItems}/{importBatchStatusResult.summary.progress.totalItems}</p>
                              ...
                              <div key={index} className="bg-gray-50 dark:bg-gray-800 p-3 rounded text-sm">
                                <div className="grid grid-cols-2 gap-2">
                                  <div>
                                    ...
                                    <p className="text-gray-600 dark:text-gray-400 font-mono text-xs">{item.itemGuid}</p>
                                  </div>
                                  <div>
                                    <span className="font-medium">Status:</span>
                                    <p className={`${item.itemStatusCode === 'BICMP' ? 'text-green-600 dark:text-green-400' : 
                                                   item.itemStatusCode === 'BIERR' ? 'text-red-600 dark:text-red-400' : 
                                                   'text-yellow-600 dark:text-yellow-400'}`}>
